...
A two-deck DJ mixer that runs in the browser: beat-matched playback, 3-band EQ
with kills, crossfader, BPM detection and sync, hot cues, loops, a music
library and headphone cue. No install, no account, no upload: your music stays
on your machine.
...
|---|---|
| PLAY / CUE | CUE while stopped sets the cue point; CUE while playing returns to it and stops |
| Overview waveform | Click to jump anywhere in the track |
| SYNC | Matches this deck's tempo and beat phase to the other deck, and keeps following its tempo. Half and double time are handled (87 syncs to 174). Press again to release |
...
| Pads 1 to 8 | Hot cues: an empty pad stores the position, a set pad jumps to it. Shift+click or right-click clears |
...
**Mixer.** Per channel: trim, high/mid/low EQ (-26 dB to +6 dB) with kill
buttons, headphone CUE, fader and level meter. In the middle: master level,
headphone level, headphone routing, crossfader and its curve (*smooth* is equal
power; *sharp* is a scratch cut). Knobs: drag up or down (Shift for fine
control), scroll, or use the arrow keys; double-click resets.
...
| 1 2 3 4 | 7 8 9 0 | Hot cues 1 to 4 (Shift clears) |

Left and Right arrows move the crossfader; Down centres it. Keys are matched by
position, so the layout is the same on non-QWERTY keyboards.
...
just what the page shows: audio reaches the master bus, beat grids match the
demo tracks, SYNC lands in phase and follows tempo changes, loops wrap inside
the audio thread, hot cues set and clear, a real WAV file decodes and
analyses, and a page reload restores the analysis and hot cues from the cache.
...
- **Analysis** (`src/analysis/`) runs in a Web Worker: band-split waveform
  peaks, and BPM from a normalised multi-band onset envelope. Candidate tempos
  come from its autocorrelation, and a comb over the whole track picks and
  refines the winner, with an off-beat check to settle half versus double
  time.
- **Mixer** (`src/audio/mixer.ts`, `engine.ts`) is plain Web Audio: gain,
  shelf and peaking filters, an equal-power crossfader, and a safety limiter
  on the master bus.

## Not yet

Key lock (tempo change without pitch change), effects, musical key detection,
recording the mix, a MIDI learn screen, and stem separation.
